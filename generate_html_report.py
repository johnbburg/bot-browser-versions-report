...
<header class="page-header" role="banner">
  ...
  <p class="meta">Directed by <a href="https://github.com/johnbburg" style="color:#93c5fd">John Brandenburg</a>
     &nbsp;·&nbsp; Analysis generated with <a href="https://claude.ai/claude-code" style="color:#93c5fd">Claude Code</a> (Anthropic)</p>
</header>
...
<aside style="background:#fef9c3; border:1px solid #fde047; border-radius:10px; padding:1rem 1.25rem; margin-bottom:1.5rem; font-size:.9rem; color:#713f12;">
  ...
  (Anthropic), directed by John Brandenburg, using only the
  <a href="https://gs.statcounter.com/browser-version-market-share" style="color:#92400e">Statcounter Global Stats</a>
  CSV exports in this project directory. No other external data sources were used.
...
<div class="exec-grid">
  <div class="exec-card">
    <div class="num red">3.6%</div>
    <p><strong>Edge 87 bot campaign.</strong> In January 2024, a single 4-year-old Edge version suddenly accounted for 3.6% of <em>all</em> US web traffic, then declined to near-zero within weeks. A characteristic bot-campaign signature.</p>
  </div>
  <div class="exec-card">
    <div class="num red">0.8%</div>
    <p><strong>Firefox 11 — ~130 versions behind current.</strong> Firefox 11 accounts for ~0.8% of all US traffic and is <em>still growing</em>. With a version lag of ~130 major versions, no plausible update path or legitimate enterprise use case explains its presence. The traffic pattern is consistent with an automated fleet.</p>
  </div>
  <div class="exec-card">
    <div class="num red">1.1%</div>
  ...
  <div class="exec-card">
    ...
    <p><strong>Chrome old-tail has approximately doubled (within Chrome).</strong> Old Chrome versions (lag ≥ 12) went from ~8.7% of Chrome's own traffic in 2022 to ~16.0% in 2025-26. As a fraction of all US web traffic, that is ~5.0% rising to ~9.7%.</p>
  </div>
  <div class="exec-card">
    <div class="num blue">109</div>
    <p><strong>Chrome 109: legitimate Windows 7 users.</strong> Chrome 109 is the last version that runs on Windows 7 and 8. It maintains ~0.5–1.0% of all US traffic. Do not hard-block Chrome 109 without other signals.</p>
  </div>
  <div class="exec-card">
    <div class="num purple">115–128</div>
    <p><strong>Firefox ESR users appear "old" but are not bots.</strong> Firefox's long-term support (ESR) channel keeps enterprise users on versions 12–25 behind current. ESR traffic (~0.1–0.4% of all traffic depending on the ESR version) is legitimate. Firefox 11 (0.5–0.8%) is not — it is far too old to be any ESR version.</p>
  </div>
</div>
...
<div class="callout info">
  ...
  A browser that is 30 versions behind is approximately <strong>2.5 years old.</strong>
...
For example, if Chrome 141 is the current stable and a visitor uses Chrome 109, their lag is 32 — meaning they're running a version approximately 2.5 years old.</p>
...
<div class="tbl-wrap">
<table>
  <tr><th>Month</th><th>Current Chrome (mode version)</th><th class="nr">Share of All US Traffic</th><th class="nr">Lag 12 Threshold (version ≤)</th></tr>
"""
    for month, ver, share in cadence_rows:
        cutoff = ver - 12
        html += f"  <tr><td>{month}</td><td>Chrome {ver}</td><td class='nr'>{share}%</td><td class='nr'>Chrome {cutoff} and older</td></tr>\n"
...
<ul style="padding-left:1.5rem; margin-bottom:1rem;">
  ...
  <li><strong>Legacy hardware/OS</strong> — Chrome 109 is the last version that runs on Windows 7 and 8.1, which some machines still run.</li>
...
<div class="callout success">
  ...
  Edge 87 appearing at 3.6% of all traffic in one month (while prior Edge 87 traffic was under 0.1%)
  is the kind of anomaly that is strongly indicative of a bot campaign.
</div>
</section>
...
<div class="tbl-wrap">
<table>
  <tr><th>Browser</th><th class="nr">2022 Avg Share (% all US traffic)</th><th class="nr">2025-26 Avg Share (% all US traffic)</th><th class="nr">Change (pp)</th></tr>
"""
    for fam in t1.index:
        ...
        html += f"  <tr><td>{fam}</td><td class='nr'>{v22:.2f}%</td><td class='nr'>{v25:.2f}%</td><td class='nr {color}'>{arrow} pp</td></tr>\n"
...
<div class="chart-block">
  ...
  <p class="chart-caption">
    <strong>Figure 1 — Browser family share, January–December 2022.</strong>
    Y-axis: share of all measured US web traffic (%). X-axis: month.
    Each line is one browser family (desktop and mobile combined for Chrome).
    Chrome led at ~58%, Safari at ~16%, Edge at ~13%. Within Chrome and Edge,
    the rapid succession of individual version releases appears as closely spaced peaks
    at the ~4-week release cadence.
  </p>
</div>
...
<div class="chart-block">
  ...
  <p class="chart-caption">
    <strong>Figure 2 — Browser family share, January 2025–January 2026.</strong>
    Y-axis: share of all measured US web traffic (%). X-axis: month.
    Chrome holds steady at ~59%. Safari fell to ~11% (down approximately 5 percentage points
    from 2022). Brave emerged at ~2.7%. Chrome for Android approximately doubled its reported
    share. Edge and Firefox remained approximately flat.
  </p>
</div>
</section>
...
<p>Three patterns in the data are strongly indicative of bot activity rather than
human browser use. They share a common signature: <em>specific old versions appearing in bulk,
often with sudden activation and cessation behavior inconsistent with normal software update patterns.</em></p>
...
under 0.1% — consistent with deeply outdated usage gradually fading away.</p>

<p>Here's what happened to Edge 87's traffic share:</p>
<div class="tbl-wrap">
<table>
  <tr><th>Month</th><th class="nr">Edge 87 Share (% all US traffic)</th><th>Interpretation</th></tr>
"""
    ...
    for _, r in e87_rel.iterrows():
        ...
        if share >= 1.0:
            ...
            note = "⚠ Anomalous rise and fall"
            cls = "hi-amber"
        else:
            note = "Background noise"
            cls = ""
        html += f"  <tr><td>{r['date'].strftime('%b %Y')}</td><td class='nr {cls}'>{share:.1f}%</td><td>{note}</td></tr>\n"
...
<div class="callout danger">
  ...
  Instead, Edge 87 went from under 0.1% to 3.6% in 3 months (a 120× increase), then declined
  to below 0.1% in 3 weeks. No organic user population behaves this way.
...
of approximately 129 versions — approximately <strong>10+ years</strong> behind current. There is no
...
<div class="tbl-wrap">
<table>
  <tr><th>Year</th><th class="nr">Avg Monthly Share (% all US traffic)</th><th class="nr">Peak Month Share (% all US traffic)</th><th class="nr">Change from 2022</th></tr>
"""
    for period, grp in ff11.groupby("period"):
        ...
        html += f"  <tr><td>{period}</td><td class='nr'>{avg:.1f}%</td><td class='nr'>{peak:.1f}% ({pk_month})</td><td class='nr'>{'8×' if period=='2025-26' else '—'}</td></tr>\n"

    html += f"""</table>
</div>

<div class="callout danger">
  <strong>🚨 Why this traffic is strongly indicative of bot activity</strong>
  Firefox 11's share has grown 8× in four years. No browser with a ~130-version lag gains
  organic users over time — this pattern strongly suggests new automated infrastructure
  being deployed with a hard-coded Firefox 11 user-agent string.
  The spike to ~0.8% of all US traffic in 2025 (approximately equalling Firefox's entire
  remaining legitimate share) strongly suggests an active, growing scraper fleet.
</div>
...
<div class="tbl-wrap">
<table>
  <tr><th>Period</th><th class="nr">Avg Monthly Share (% all US traffic)</th><th>Notes</th></tr>
"""
    ...
    for period, grp, note in [
        ...
        ("2025 (Jan–Dec)", ff118_2526[ff118_2526["date"] < "2026-01-01"], "Still at ~1.0%+ — elevated; probable non-human traffic"),
        ("Jan 2026", ff118_2526[ff118_2526["date"] == "2026-01-01"], "Drops to exactly 0% in one month"),
    ]:
        ...
        html += f"  <tr><td>{period}</td><td class='nr {cls}'>{avg:.1f}%</td><td>{note}</td></tr>\n"

    html += f"""</table>
</div>

<div class="callout danger">
  <strong>🚨 A strongly anomalous pattern</strong>
  A browser version released in 2023 that maintains approximately 1% of all US traffic through
  all of 2024 and 2025 — without the gradual decline expected from normal user adoption —
  and then drops to <em>exactly zero</em> in one month, is highly consistent with automated
  traffic: either a campaign shutting down, migrating to a newer version, or a data-collection
  methodology change — but not with any organic human browser update pattern.
  By late 2025, Firefox 118 was approximately 25 versions behind current stable.
</div>
...
<div class="chart-block">
  ...
  <p class="chart-caption">
    <strong>Figure 3 — Chrome old-version traffic over time (lag ≥ 12, 99.5th-percentile reference).</strong>
    Y-axis: share of all US web traffic (%) from Chrome versions more than approximately
    one year behind current stable. X-axis: month within each year. Each colored line is
    one calendar year. The October 2025 spike (~29.6%) was approximately 3× the 2025-26
    annual average (~9.7%).
  </p>
</div>
...
<div class="tbl-wrap">
<table>
  <tr><th>Version</th><th class="nr">Share, Oct 2025 (% all US traffic)</th><th>Approx Age</th><th>Assessment</th></tr>
  <tr><td>Chrome 141</td><td class="nr">19.0%</td><td>Current stable</td><td class="hi-green">Normal ✓</td></tr>
  <tr><td class="hi-amber">Chrome 125</td><td class="nr hi-amber">12.8%</td><td>~17 months old (released May 2024)</td><td>Elevated lag; probable non-human traffic. Also spiked in Dec 2024. Possible automation or pinned fleet.</td></tr>
  <tr><td class="hi-amber">Chrome 130</td><td class="nr hi-amber">11.1%</td><td>~5 months old</td><td>Elevated lag; consistent with delayed update cycle. Could be enterprise or extended-stable users.</td></tr>
  <tr><td>Chrome 117</td><td class="nr">0.7%</td><td>~1 year old</td><td>Elevated lag; probable non-human traffic</td></tr>
  <tr><td class="hi-red">Chrome 109</td><td class="nr hi-red">0.5%</td><td>~2.5 years old</td><td>Windows 7 and 8 legacy (likely legitimate)</td></tr>
  <tr><td class="hi-red">Chrome 79</td><td class="nr hi-red">0.4%</td><td>~5 years old</td><td>Strong bot signal</td></tr>
  <tr><td class="hi-red">Chrome 83</td><td class="nr hi-red">0.3%</td><td>~4.5 years old</td><td>Strong bot signal</td></tr>
</table>
</div>
...
<p>Throughout this section, <strong>"old-version traffic"</strong> means traffic from browsers
with a version lag of 12 or more — approximately one year or more behind the current stable
release (99.5th-percentile reference). This is the lag ≥ 12 tail visible in the bucket charts above.</p>
...
We measure this with the <strong>Herfindahl-Hirschman Index (HHI)</strong>, a standard
concentration metric — a higher score means traffic is more concentrated on fewer specific versions.</p>

<div class="callout info">
  <strong>Interpreting HHI scores (range 0–10,000)</strong>
  0–1,000 — Traffic distributed across many versions; consistent with organic update diversity.<br>
  1,000–2,500 — Moderately concentrated; warrants monitoring.<br>
...
<div class="chart-wide">
  ...
  <p class="chart-caption">
    <strong>Figure 4 — Version concentration (HHI) of old-version traffic, Chrome / Edge / Firefox, all years.</strong>
    Y-axis: Herfindahl-Hirschman Index (HHI, 0–10,000) applied to the lag ≥ 12 tail of each browser.
    A higher HHI means old-version traffic is more concentrated on fewer specific versions —
    a pattern more consistent with automated fleets using hard-coded user-agent strings than
    with organic slow-updating users. X-axis: month. Each colored line is one calendar year.
    The red dashed line marks HHI = 2,500, the conventional "concentrated" threshold.
    Edge's 2024 spike corresponds to the Edge 87 campaign; Firefox's upward trend reflects
    the growing Firefox 11 and Firefox 118 fleets.
  </p>
</div>
...
<table>
  <tr>
    ...
    <th class="nr">Concentration Score (HHI)</th>
    <th class="nr">Top 1 Version (% of lag ≥ 12 tail)</th>
    <th class="nr">Top 3 Versions (% of lag ≥ 12 tail)</th>
  </tr>
"""
    ...
    for fam in ["Chrome","Edge","Firefox"]:
        for period in ["2022","2025-26"]:
            ...
            html += f"  <tr><td>{fam}</td><td>{period}</td><td class='nr {hhi_cls}'>{hhi:,.0f}</td><td class='nr'>{t1v:.1f}%</td><td class='nr {t3_cls}'>{t3v:.1f}%</td></tr>\n"
...
<p style="font-size:.875rem; color:#666">All lag values in this section use the <strong>99.5th-percentile reference version</strong>
— the version at or below which 99.5% of a browser family's traffic falls in a given month.
This approach handles small fractions of pre-release (Beta/Canary) traffic more robustly
than using the single highest-share version.</p>

<div class="chart-block">
  <img src="{imgs['bucket_cmp']}" alt="Version lag bucket distribution comparison">
  <p class="chart-caption">
    <strong>Figure 5 — Version lag bucket distribution by browser, 2022 vs 2025-26.</strong>
    Y-axis: share of each browser's own traffic (%) in each lag bucket. Bars within a browser
    sum to 100%. X-axis: lag bucket. Two bars per cluster: 2022 (blue) and 2025-26 (orange).
    Note: these are percentages <em>within each browser's traffic</em>, not of all US traffic.
    Edge remains concentrated in the 0–2 lag bucket. Chrome and Firefox have shifted
    toward older buckets between 2022 and 2025-26.
  </p>
</div>
  ...
  <div class="thresh-card">
    ...
    <div class="tc-body">
      ...
      <ul>
        ...
        <li><span class="caution">Lag 12–17</span> — Elevated lag. Soft-challenge or rate-limit. (~7% of Chrome)</li>
      ...
      <p><strong>Never hard-block on lag alone:</strong> Chrome 109 (Windows 7 and 8 legacy users, ~0.5–1.0% of all traffic)</p>
    </div>
  </div>
  <div class="thresh-card">
    ...
    <div class="tc-body">
      ...
      <ul>
        ...
        <li>Edge 15, 18, and 19 — legacy EdgeHTML browser, EOL 2021</li>
      </ul>
    </div>
  </div>
  <div class="thresh-card">
    ...
    <div class="tc-body">
      ...
      <ul>
        ...
        <li><span class="block">Lag 60+</span> — Hard-block. Versions like Firefox 72, 78, 44 (2016–2020). Almost certainly non-human traffic.</li>
      </ul>
...
"False positive rate" (FP rate) refers to the percentage of browser traffic in a given category
that would be blocked or challenged — lower is better for user experience.</p>

<div class="callout warning">
  <strong>⚠ FP rates are computed over all traffic, including non-human traffic</strong>
  StatCounter data cannot separate human from automated visits. The FP rates below are
  calculated over <em>all</em> traffic in each browser category, including any bot traffic
  already present in that category. The true false positive rate for <em>human</em> visitors
  is therefore lower than the figures shown — particularly for Chrome and Firefox, where
  the old-version tail contains substantial non-human traffic. Edge FP rates are more
  reliable in this respect, as its old-version tail is small and predominantly non-human.
</div>
...
<table>
  ...
  <tr>
    ...
    <td>Chrome 70 released Oct 2018 — 7+ years old. Traffic at ~0.1% is consistent with non-human traffic.</td>
  ...
  <tr>
    <td>Edge</td>
    <td>Block Edge 87, and Edge 15, 18, 19</td>
    <td>Edge 87 = confirmed campaign; EdgeHTML versions (15, 18, 19) = EOL 2021</td>
    <td class="hi-green">Near zero</td>
  </tr>
  <tr>
    <td>IE</td>
...
<div class="tbl-wrap">
<table>
  <tr><th>Browser</th><th class="nr">≤1% FP: min lag</th><th class="nr">≤2% FP: min lag</th><th class="nr">≤5% FP: min lag</th></tr>
  <tr><td>Chrome (2025-26)</td><td class="nr hi-red">&gt;24 (no threshold)</td><td class="nr hi-red">&gt;24</td><td class="nr hi-amber">Lag ≥ 25–30*</td></tr>
  <tr><td>Edge (2025-26)</td><td class="nr hi-amber">Lag ≥ 21</td><td class="nr hi-green">Lag ≥ 9</td><td class="nr hi-green">Lag ≥ 3</td></tr>
  <tr><td>Firefox (2025-26)</td><td class="nr hi-red">&gt;24 (bot-inflated)</td><td class="nr hi-red">&gt;24</td><td class="nr hi-red">&gt;24*</td></tr>
  <tr><td>Safari</td><td colspan="3">Use conservative hard-block list only — no lag rule</td></tr>
</table>
</div>
<p style="font-size:.875rem; color:#666">* Chrome and Firefox cannot achieve a sub-5% FP threshold at any lag ≤ 24 because their old-version tails contain substantial non-human traffic (see caveat above). Chrome lag ≥ 25 is a reasonable soft-challenge starting point. For Firefox, prefer specific version hard-blocks over a blanket lag rule.</p>
</div>
...
<div class="tbl-wrap">
<table>
  <tr><th>Browser</th><th>Threshold</th><th class="nr">Expected FP Rate</th><th>Justification</th></tr>
  <tr><td>Chrome</td><td>Lag ≥ 12 (~1 year old)</td><td class="nr hi-amber">~16.0% of Chrome traffic</td><td>HHI rising; Oct 2025 spike shows bot-heavy months. Pair with rate signals to reduce FP.</td></tr>
  <tr><td>Edge</td><td>Lag ≥ 12</td><td class="nr hi-green">&lt;1.0% of Edge traffic</td><td>Edge users are nearly always current. Very safe to block.</td></tr>
  <tr><td>Firefox</td><td>Lag ≥ 12 + explicit version list</td><td class="nr hi-amber">~6.0% of Firefox* (but mostly non-human)</td><td>Old Firefox tail is heavily concentrated on known bot versions. Most "FP" here are non-human traffic.</td></tr>
  <tr><td>Safari</td><td>Hard-block list only</td><td class="nr hi-green">~0.2% of Safari</td><td>Safari 12 and older, fake WebKit UAs</td></tr>
</table>
</div>
</div>
...
<div class="tbl-wrap">
<table>
  <tr><th scope="col">Safari Version</th><th scope="col">Tied To</th><th scope="col" class="nr">Avg Share (2025-26, all sub-versions)</th><th scope="col">Under lag-based rule...</th></tr>
  <tr><td class="hi-green">Safari 18</td><td>iOS 18 (Sep 2024)</td><td class="hi-green nr"><strong>6.4%</strong> (largest by far)</td><td class="hi-red">⚠ Lag 8 — flagged as "old" (Aug 2025 onward)</td></tr>
  <tr><td>Safari 17</td><td>iOS 17 (Sep 2023)</td><td class="nr">2.1%</td><td class="hi-amber">Lag 9 — flagged</td></tr>
  <tr><td>Safari 16</td><td>iOS 16 (Sep 2022)</td><td class="nr">1.2%</td><td class="hi-amber">Lag 10 — flagged</td></tr>
  <tr><td class="hi-green">Safari 26</td><td>iOS 26 and macOS 26 (2025)</td><td class="nr">0.7%</td><td class="hi-green">Current — fine ✓</td></tr>
  <tr><td>Safari 15</td><td>iOS 15 and macOS 12</td><td class="nr">0.6%</td><td class="hi-red">Lag 11 — blocked</td></tr>
  <tr><td>Safari 14</td><td>iOS 14 and macOS 11</td><td class="nr">0.2%</td><td class="hi-red">Lag 12 — blocked</td></tr>
  <tr><td>Safari 13</td><td>iOS 13 and macOS 10.15</td><td class="nr">0.1%</td><td class="hi-red">Lag 13 — blocked</td></tr>
</table>
</div>
...
<div class="callout warning">
  ...
  from August 2025 onward when iOS 26 and macOS 26 launched. This mid-period shift means the same
...
<div class="callout warning">
  ...
  If Firefox 11 has 0.8% of StatCounter page views, that does not mean it will appear
  at 0.8% of entries in your access log. The actual percentage depends heavily on
...
relative to the number of actual bot instances. The 0.8% "Firefox 11 share" could
...
shows Firefox 11 at 0.8% of page views, the actual percentage of HTTP requests
...
<div class="callout warning">
  ...
  All bot inferences are probabilistic — based on patterns (extreme age, sudden activation
  and cessation, concentration on specific versions) inconsistent with human behavior. To confirm bot
...
<h3>Data Source</h3>
<p>All data in this report is sourced from
<strong><a href="https://gs.statcounter.com/browser-version-market-share">Statcounter Global Stats</a></strong>
(© Statcounter 1999–2026), licensed under a
<a href="https://creativecommons.org/licenses/by-sa/3.0/">Creative Commons Attribution-Share Alike 3.0 Unported License</a>.
Use of this data is credited to Statcounter as required by their terms.</p>
<p>StatCounter measures <strong>page views</strong> — not unique visitors or raw HTTP requests —
collected from a network of over 1.5 million websites representing approximately 5 billion
page views per month. The data reflects JavaScript-rendered page views only; traffic from
automated clients that do not execute JavaScript is not captured. Statistics are subject to
quality assurance revision for 45 days from initial publication.</p>
<p>Four monthly CSV exports covering January 2022 through January 2026 were downloaded
from the StatCounter "US browser version" report and loaded into the analysis pipeline.
Files were converted from wide format (one column per browser version) to long format with one
...
<footer>
  <p>Directed by <a href="https://github.com/johnbburg">John Brandenburg</a>
     &nbsp;·&nbsp; Analysis generated with <a href="https://claude.ai/claude-code">Claude Code</a> (Anthropic)</p>
  <p style="margin-top:.5rem">Data: <a href="https://gs.statcounter.com/browser-version-market-share">Statcounter Global Stats</a>
     © Statcounter 1999–2026, licensed under
     <a href="https://creativecommons.org/licenses/by-sa/3.0/">CC BY-SA 3.0</a>.
     Period: January 2022 – January 2026 (US, monthly browser version).</p>
